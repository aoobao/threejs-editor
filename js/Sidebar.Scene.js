...
Sidebar.Scene = function (editor) {
	...
	var backgroundColor = new UI.Color().setValue('#' + DEFAULT_BACKGROUND.toString(16)).onChange(onBackgroundChanged);
	...
	function refreshUI () {
		...
		(function addObjects (objects, pad) {
			...
			for (var i = 0, l = objects.length; i < l; i++) {
				...
				if (!object.userData.disable) {
					var option = buildOption(object, false);
					option.style.paddingLeft = (pad * 10) + 'px';
					options.push(option);
				}

				// 如果当前object是设备,不再放入下级object
